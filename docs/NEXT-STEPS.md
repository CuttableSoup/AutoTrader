...
> **Update 2026-09-16, still the same day: recommendation 2's "one more pre-registered attempt" path is now closed.** Instead of a fifth narrow hypothesis, a frozen 1,368-configuration brute-force grid was run across price/technical, fundamental and event-based signal families (including two H1-H4 did not test: SF1 fields beyond gross profit/issuance, and Sharadar insider/event data), corrected for overfitting with Deflated Sharpe Ratio and Probability of Backtest Overfitting instead of pre-registration. It also fails: DSR -0.635, PBO 0.630 (worse than a coin flip). See [BRUTEFORCE-RESULT.md](BRUTEFORCE-RESULT.md). Option A below (accept and stop on single-name equities) is the path all of that data supports.

> **Update 2026-09-16, the same day again: a strategy-family pivot passed, including the sealed hold-out.** Rather than search equities further, the owner pivoted to time-series trend-following on a fixed 18-ETF universe (equities, rates/credit, commodities, currencies) -- one pre-registered specification (Moskowitz-Ooi-Pedersen 2012), not another grid. Development: Deflated Sharpe +0.318, 5/5 out-of-sample folds positive. The genuinely unseen hold-out year (2025-09-16..2026-09-16, never touched by signal design or any prior test) returned +5.08%, Sharpe ~1.4 -- directionally confirming, not contradicting, development. See [TSMOM-RESULT.md](TSMOM-RESULT.md) for the full result, the four bugs a code review caught and fixed before this touched live data, and the robustness checks (broad asset-class attribution, cost-doubling survival, leave-one-fold-out) run before spending the hold-out. This is the first strategy in the project's history to clear a pre-registered gate. What's next is Phase 2: wiring this into the live C++ system (new StrategyEngine path, schema, risk caps, config) -- not designed yet, and per the owner's explicit position, paper trading should wait until that's built and the strategy is running for real, not before.
